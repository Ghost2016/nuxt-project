...
module.exports = async (ctx, next) => {
  ...
  request.checkBody = rule => check(rule, 'body')
  request.checkQuery = rule => check(rule, 'query')
  request.checkParams = rule => check(rule, 'params')

  const check = (rules, type) => {
    // ['name', 'email']
    for (const [fieldName, fieldValidator] of Object.entries(rules)) {
      // 执行验证，validator 默认不处理 undefined值
      const val = request[type] && request[type][fieldName] || ''
      let pass = false
